import { applyDecorators, HttpCode, Get, Post, Put, Delete } from '@nestjs/common';
import { ApiResponse, ApiBody, ApiOperation } from '@nestjs/swagger';
import { BadRequestDto, UnauthorizedDto, ForbiddenDto, NotFoundDto, CompletedDto, ValidationDto, DuplicateDto } from './default.dto';
...
export function ApiGet(url: string = '/', summary: string = '', response?: any) {
  return applyDecorators(Get(url), ApiOperation({ summary: summary }), ApiResponseDefault(response));
}
